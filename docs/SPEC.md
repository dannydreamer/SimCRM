...
2. **Casting is fully complete** — `castingSentAt` is set, and filled slots equal total slots, where
   ...
   This is the **Step 2** measure and it is the only thing that gates READY. The Tech's screens never display it as a fraction — they show three states, not numbers (§7.7) — but their ✓ is this same test, so nothing on the workshop page can disagree with this condition.
...
The alert carries the day count and **both** halves of the gate: the unmet conditions from §4.3 and the outstanding משימות נוספות from §4.3.1, as two separate lists. A `סדנה חדשה` reports **only איתור צרכים** — no minor tasks either: everything downstream is genuinely unmet too, but none of it can be done before the needs assessment is, so listing the rest is noise.
...
**משימות נוספות overlay (§4.3.1).** `[code]` The checklist's sixth line is `משימות נוספות (x/y)`, followed by `← ` and the name of the next outstanding task. The line is a button; so is a standalone chip shown for the statuses that have no checklist (`סדנה חדשה`, `בתהליך סגירה`, `סגור`) — a Tech books תיקתק long before איתור צרכים is marked, so the list must not be reachable only from `בוצע איתור צרכים` / `מוכן`.
