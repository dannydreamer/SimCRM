...
> **Cancelled workshops are excluded everywhere on these pages** — the list card's workshop count and last date, the detail page's room totals (סה"כ חדרים שבוצעו / חדרים מתוכננים), and each group row's count and last date. A cancelled workshop keeps its rooms, so without this an organization whose only workshop was cancelled reported a room done. Group rows' last date also skips future workshops (`0746270`); the list card's does not. `[code]`
